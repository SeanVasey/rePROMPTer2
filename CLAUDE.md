...
- Keep it dark, glossy, high-contrast, restrained. No purple gradients, no generic
  "AI slop" aesthetics.
- **Logo / app icon assets:** `public/icon.svg` is the canonical app icon (favicon / PWA
  / `apple-touch-icon` source) and the logo shown in the README; `app/components/Logo.tsx`
  is the in-app header emblem. Keep these two in sync — when the brand mark changes, update
  both so the header, favicon, and PWA icon stay consistent.
